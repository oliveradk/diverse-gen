...
param_space = ng.p.Dict(
    aux_weight=ng.p.Log(lower=1e0, upper=1e3),
)
...
get_conf_name = lambda config: (config[0][0], config[1][0], config[2])
configs = [config for config in configs if get_conf_name(config) not in to_remove]
...
for (env_name, env_config), (loss_name, loss_config), mix_rate in tqdm(configs, desc="Sweeping"):
    ...
    executor = get_executor(sweep_dir, mem_gb=16 if env_name not in ["celebA-0", "multi-nli"] else 32)
